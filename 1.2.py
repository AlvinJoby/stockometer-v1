...
def preprocess_data(data):
    ...
    numeric_columns = ['OPEN', 'HIGH', 'LOW', 'CLOSE','VWAP','52W H','52W L', 'VOLUME', 'NO. OF TRADES']
...
def daily_returns(data):
    ...
    print("\nAbout Daily Returns:")
    hdr_value=data['Daily Return'].max()
    hdr_date=data['Daily Return'].idxmax()
    ldr_value=data['Daily Return'].min()
    ldr_date=data['Daily Return'].idxmin()
    print(f"\nHighest Daily Return marked is {hdr_value} on {hdr_date}")
    print(f"Lowest Daily Return marked is {ldr_value} on {ldr_date}")
...
def cumulative_returns(data):
    ...
    """Calculate cumulative retruns."""
    data['Cumulative Return'] = (1+data['Daily Return']).cumprod()

    print("\nAbout cumulative returns: \n")
    final_value = data['Cumulative Return'].iloc[-1]
    print(f"\nIf 1rs invested it would have become {final_value:.3f}rs\n")
    total_returns = (final_value/1)*100
    print(f"Total returns over this period is {total_returns:.2f}%")


def main():
    csvfile = input("Enter the name of file: ")
